...
key_t sem_inspector1_key , sem_inspector2_key;
int shm_id = -1, sem_id = -1, msg_id = -1;
int sem_inspector1_id = -1 , sem_inspector2_id = -1 ;
void cleanup() {
...
int main(int argc, char *argv[]) {
    ...
    *file_counter = 0; // Initialize counter
    ...
    char sem_inspector1_key_str[20] , sem_inspector2_key_str[20];
    ...
    for (int i = 0; i < config.NUM_MOVERS; i++) {
        if ((movers_pid[i] = fork()) == 0) {
            execl("/home/adduser/ENCS4330/Projects/Project2/File-Management-Simulation/projectCode/bin/mover", "mover", argv[1], NULL);
